...
- `@/` alias (group 4): must sort by **layer** first — `domain` → `integration` → `presentation` (inward-out, mirrors the [layer model](../architecture/architecture.md)) — then alphabetically by full path within a layer.
...
- must import React bindings by **name**, never a default namespace — `import { useState, type ReactNode } from "react"`; no `import React from "react"`, no `React.*` UMD reference ([code-organization.md](code-organization.md#react-types--import-named-never-the-umd-namespace)).
