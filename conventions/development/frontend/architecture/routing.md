...
> Purpose — one routing model across every app, with cross-cutting behavior added in one place. Sits with [architecture.md §Routing & responsive surfaces](architecture.md) (the place↔page / action↔modal doctrine).
...
- must give every **place** a route + a `*Page` (deep-linkable / refreshable / shareable); every **action** a routeless `*Modal` (`useState`-driven), presented as `ResponsiveModal` (Modal ↔ BottomSheet). Full doctrine: [architecture.md §Routing & responsive surfaces](architecture.md).
...
- **baseline component names** (`AppRoot` · `AppLayout` · `AppErrorBoundary` · bare `NotFound` / `DocumentTitle`) — see [naming.md § App-shell baseline components](../code-style/naming.md).
