...
Exhaustive for HTML's global attributes plus the ARIA surface in use. `data-state` and `aria-label` lead both trees.

| Attribute | Means | Verdict |
|---|---|---|
| `class` | the utility string styling the element | `use` |
| `id` | a unique handle, generated by `useId()` | `use` |
...
- must generate every `id` with `useId()`, then pass it to whatever references it.
