...
- must reach for `AppError` first; `Result<T, TFailure>` earns itself only where the caller switches on cases.
...
- must author an error through a catalog — SDK `AppErrors.{Kind}(…)`, app `CodeErrors.*`.
