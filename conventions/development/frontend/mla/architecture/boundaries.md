...
> The app's outer edge — what stays inside it, what extracts to the SDK, how it is packaged and served.
...
- must start at the screen, and extract a shared component or hook **within the app** only once a second
...
- must proxy `/api` to the backend's even HTTPS port (`secure: false`, `changeOrigin: false`) —
...
- [between our own frontends](../../hla/hla.md) — the scope above, once a second frontend exists
