...
| declares an operation as data | `*Descriptor` | `SortDescriptor` |
| the outcome of a fallible call | `*Result` | `ClipboardWriteResult` |
| the caller's knobs for one call or module | `*Options` | `ApiClientOptions` |

- must pair `*Options` with the one call or module it configures, and name it for that — `ApiClientOptions`.
- must return a `*Result` from a call that can fail without throwing, never a bare boolean.
...
- must declare the **component** as a `.vue` file whose template is `<slot />` ([provider](provider.md)).
...
- [component catalog](component-catalog.md) — the kinds that render, and the suffix each one fixes
- [provider](provider.md) — the component kind that installs a seam for a subtree
