...
- must not be the capability — the seam it installs is a `.ts` contract ([headless suffixes](headless-suffixes.md)).
...
- [headless suffixes](headless-suffixes.md) — the `.ts` seam vocabulary a provider installs
- [primitive](primitive.md) — the rendering-level providers that ship in `foundation/primitives/`
- [hooks](hooks.md) — the `use{Capability}()` composable a provider is read through
