...
- must name the role in the prefix; only a wire **write** payload breaks the pattern, as a `*Request`.
...
- must name it `Descriptor` not `Spec` (`.spec` collides with tests) and `Catalog` not `Registry`.
- must send a `*Request` on a write, never a read shape — a read carries server-owned fields
...
- must co-locate a type in the slice that **owns** it — a `*Model` in `domain/`, a `*Dto` / `*ApiRequest`
  with its client in `integration/` ([architecture](../../architecture/architecture.md)).
...
- [architecture](../../architecture/architecture.md) — the slice a model is declared in
