...
- must expose an `as` prop for the rendered element when the semantic tag varies ([styling](../platform/styling.md)).

### Component name

- must end `*Layout` for chrome around a router outlet, `*Shell` for the app frame — one `*Shell` per app.
- must keep a widely-known bare name bare — `Box` · `Flex` · `Grid` · `Stack` · `Center`.
...
- [styling](../platform/styling.md) — tokens, `cn()`, and the variant files a layout uses
