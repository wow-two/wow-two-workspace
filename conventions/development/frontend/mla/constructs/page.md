...
- must be lazy-imported from the route table ([routing](../domains/routing/routing.md)).
...
/** Renders the code builder screen. */
...
- must read route params and search state through the router hooks ([routing](../domains/routing/routing.md)).
...
- must open an [overlay](overlay.md) for an action, and route to a sibling screen for a place.
- must call `application/` hooks for data, never `integration/` ([architecture](../architecture/architecture.md)).
...
- [routing](../domains/routing/routing.md) — the route table, params, and code-splitting
