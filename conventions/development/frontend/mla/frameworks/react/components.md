...
- must declare a component as an exported `function`, one per folder
...
- **class component** — reach for a function plus hooks; no hook runs in a class, so every SDK composable, every
