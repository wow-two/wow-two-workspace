...
| `ApiError` | type | thrown from `onSubmit`; drives field-error mapping (`foundation/http`) |

---

## Engine pin [REQUIRED]

- must pin the engine once per app — `src/form.ts` re-exports one adapter subpath, `@/form` is what screens import.
...
- must call `reset(loadedEntity)` from an edit screen's Discard, never bare `reset()`.
...
- must look a control up by its [kind](../../constructs/component-catalog.md) and read its own
