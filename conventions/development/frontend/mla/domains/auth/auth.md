...
> Purpose — a screen reads a status and a user; how the credential was obtained stays behind the strategy.
...
- must resolve the current user once on mount, deduped through one shared in-flight promise.
...
- must route a transport 401 through the module-scope bridge, which flips the session.
- must let a route guard await an unsettled session before it allows or redirects.
- must publish every transition on the bridge — a non-component seam reads it there, never through the tree.
- must return a disposer from every bridge subscription ([hooks](../../constructs/hooks.md) § *Lifecycle rules*).
- must type the user generically; its shape is the app's, never the contract's.
- must ship no UI — gates, splash screens and sign-in pages stay app-side.
- must read a browser global inside an action, never at factory time, so the module imports under SSR.
...
- [routing](../routing/routing.md) — the guard that reads the settled session
- [provider](../../constructs/provider.md) — the kind that installs the session for a subtree
