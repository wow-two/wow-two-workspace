...
| `enum` | camelCase `string` | enum const-type ([enums](../../../lla/components/enums.md)) | wire value **is** the enum value — no mapping |
...
Dates convert at the **HTTP client boundary** ([state-and-data.md](../data/state-and-data.md)), globally — never in a per-type mapper.
...
> `Temporal.*`. A mapper exists only to reshape (flat ↔ rich), never to parse a date ([models.md](../../components/data/models.md)).

---

## Enums

- wire value = the const-object's camelCase value = the TS enum value → **identity**, no mapping. Backend must serialize camelCase (`JsonStringEnumConverter(JsonNamingPolicy.CamelCase)`). Full pattern: [enums](../../../lla/components/enums.md).
...
- required → `field: T` · optional → `field?: T`. Never `T | null` and never emit `null` on the wire — the backend omits null keys ([models.md](../../components/data/models.md)).
