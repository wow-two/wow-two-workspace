...
Non-component files co-located in a folder use **descriptive PascalCase + role suffix**:

| Suffix | Use | Examples |
|---|---|---|
| `*Extensions.ts` | Dependency-free logic over a type — built-in, external, or our own (see [extensions](../../components/extensions.md)) | `DateExtensions.ts`, `FormExtensions.ts` |
| `*Styles.ts` | Shared `tailwind-variants` configs (see [styling](../../../mla/platform/styling.md)) | `ButtonStyles.ts`, `InputStyles.ts` |
...
A swappable app-side seam over a **client-side resource** — `localStorage`, `sessionStorage`, a Redux store, an in-memory map — is a **`*Broker`**: the contract `StorageBroker`, its instances `localStorageBroker` / `memoryBroker`, and the injected dependency a `broker` param / option. It borrows the backend **`Broker`** term ([components](../../../../backend/dotnet/mla/components/components.md) — "app-side seam over an external dependency"), so the same word names the same idea on both sides of the stack. Reach for it only for a resource seam; dependency-free logic stays `*Extensions`.
...
**Bare (not `App*`):** a **page** the router renders keeps a plain page name — `NotFound` (the `*` catch-all), the `*Page` places — and a **utility** stays descriptive — `DocumentTitle`, `createAppRouter`. `App*` marks the singleton *frame* pieces, not every routing file. See [routing.md](../../../mla/domains/routing/routing.md).
...
Prop **names** follow a fixed vocabulary so any component reads the same way. (Prop **shape** rules — `readonly`, no-destructure, `interface` — live in [components](../../../mla/components/components.md).)
...
- [enums](../../components/enums.md) — enums-as-`enum` + label Record; domain enum vs. UI value-set split
- [components](../../../mla/components/components.md) — one-component-per-folder; prop **shape** rules (`readonly`, no-destructure) that pair with the prop **names** above
- [extensions](../../components/extensions.md) — `*Extensions` const objects

---

## Banned names

- must not name a file, type, or object `Helper` · `Helpers` · `Util` · `Utils` · `Common` · `Manager` — a name describing no role names nothing, and the folder becomes the place work goes to be lost.
...
- the ban is language-wide, matching the backend ([naming](../../../../backend/dotnet/lla/notation/naming/naming.md)); `*Extensions` is the shared replacement on both sides.

```typescript
// ❌ names no role — every later utility lands here too
// FormHelpers.ts
export function toFieldErrors(problem: ProblemDetails) { }

// ✅ names the domain it extends
// FormExtensions.ts
export const FormExtensions = {
  toFieldErrors(problem: ProblemDetails) { },
} as const;
```
