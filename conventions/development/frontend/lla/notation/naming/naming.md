...
- must name every `.ts` and `.tsx` file `PascalCase`, matching its primary export.
...
- the casing is ecosystem-wide, matching the [backend](../../../../backend/dotnet/lla/notation/naming/naming.md).
...
- must suffix a shared `tailwind-variants` config `*Styles.ts` ([styling](../../../mla/platform/styling.md)).
...
- the word matches the backend `Broker` ([components](../../../../backend/dotnet/mla/components/components.md)).
...
- must leave a router-rendered page on a plain page name (`NotFound`, the `*Page` places) and a utility
  descriptive (`DocumentTitle`, `createAppRouter`) — see [routing](../../../mla/domains/routing/routing.md).
...
- the ban is language-wide — the [backend](../../../../backend/dotnet/lla/notation/naming/naming.md) bans them too.
...
- which **suffix** names which component kind, and the bare names that take none
  → [component catalog](../../../mla/constructs/component-catalog.md) § *Suffix routing*
...
- a seam a component consumes — `*Client` · `*Bus` · `*Policy`
  → [headless suffixes](../../../mla/constructs/headless-suffixes.md)
