...
- `@/` alias (group 4): must sort by **layer** first — `domain` → `integration` → `presentation`, mirroring
  the [layer model](../../../mla/architecture/architecture.md) — then alphabetically by full path.
...
- must import React bindings by **name**, never a default namespace — no `import React from "react"` and no
  `React.*` UMD reference ([style](style.md)).
...
- must import a symbol from the barrel that owns it, never from a second barrel that republishes it — a slice
  ...
  ([hooks](../../../mla/constructs/hooks.md) § *One export site*).
