...
Exhaustive for HTML sectioning content, plus the two landmark roles with no element behind them.

| Element | Means | Verdict |
|---|---|---|
| `<main>` | the page's primary content — one per screen | `use` |
...
| `<section>` | a themed region, only when a heading names it | `use with care` |
...
| `<section>` with no accessible name | a landmark announced as "region" and nothing more | `banned` |
| a second `<main>` on one screen | two answers to "where does the content start" | `banned` |
...
- must give each routed screen one `<main>`, holding everything that is not shell
  ([routing](../../../mla/domains/routing/routing.md)).
- must name every repeated landmark with `aria-label` — two unlabelled `<nav>`s are indistinguishable.
- must reach for `<section>` only when a heading names it, and `<div>` otherwise ([headings](headings.md)).
- must scope `<header>` / `<footer>` to their nearest section — inside `<article>` they stop being page landmarks.
- must keep the shell's landmarks in the layout component, never re-declared per screen
  ([architecture](../../../mla/architecture/architecture.md)).
...
- **`<section>` with no accessible name** — reach for `<div>`, or add the heading; an unnamed region is announced as
  "region" with no further detail, which adds a stop to the landmark list and no information at it.
- **a second `<main>` on one screen** — reach for `<section>`; the skip-to-content shortcut targets the first
...
- **`<div class="page">` where `<main>` belongs** — reach for `<main>`; a screen-reader user loses the one jump that
  skips the header and nav on every screen, and re-reads the chrome on each navigation.
...
- [architecture](../../../mla/architecture/architecture.md) — which layer owns the shell
