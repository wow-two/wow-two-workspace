...
Exhaustive. `h2` dominates both trees, `h1` appears three times — one per routed screen, which is the intended shape.
...
| more than one `<h1>` per routed screen | two competing page subjects | `banned` |
| a skipped level (`<h2>` → `<h4>`) | a gap the outline reads as a missing section | `banned` |

- must give each routed screen exactly one `<h1>` naming its subject
...
- **a second `<h1>` on one routed screen** — reach for `<h2>`; two page subjects leave assistive technology with no
