...
- must derive every value set from a `const` object, never from a bare union ([enums](../components/enums.md)).
- must declare an object shape as an `interface`, and reserve `type` for a union or a derivation.
- must mark every member of a props `interface` `readonly` ([components](../../mla/components/components.md)).
- must model an absent field as `field?: T`, never `T | null` ([type mapping](../../mla/domains/api/type-mapping.md)).
- must keep a `class` to an `Error` subclass or a chained builder — a value set is a `const` object.
- must declare a module-level operation as a `function`, leaving the arrow for an inline callback.
- must let a folder's lowercase `index.ts` be its only public surface ([naming](../notation/naming/naming.md)).
...
- **`enum` · `const enum`** — reach for a `const` object ([enums](../components/enums.md)); the member is nominal, so
  the wire string it equals will not assign to it, and a mapper appears at every read boundary. `const enum` also
  inlines its members, which `isolatedModules` cannot do one file at a time.
- **bare string-literal union** — reach for that same `const` object ([enums](../components/enums.md) § *Use*); a
...
- **`any`** — reach for `unknown` and narrow it; checking stops at the annotation, and every error inside that value
  moves to runtime.
- **`T[]` · `readonly T[]`** — reach for `ReadonlyArray<T>` ([type mapping](../../mla/domains/api/type-mapping.md));
  the bracket form breaks the `IReadOnlyList<T>` pairing and hands a prop the mutators it must not expose.
...
- [constants](../components/constants.md) · [extensions](../components/extensions.md) — the other service-free roles
- [components](../../mla/components/components.md) — the roles that need an app around them
- [C# constructs](../../../backend/dotnet/lla/constructs/constructs.md) — the backend sibling this mirrors
