...
`pointer-events-none`, `cursor-not-allowed`, `cursor-pointer` and `select-none` lead the group.
...
| `cursor-pointer` on a `<button>` or `<a>` | a cursor those elements already have | `banned` |
...
- must not add `cursor-pointer` to an element that already carries the role — it signals the wrong element.
...
- **`cursor-pointer` on a `<button>` or `<a href>`** — reach for the bare element; both already carry it, and adding
  the class is usually how a clickable `div` gets mistaken for a control ([flow](../html/flow.md)).
