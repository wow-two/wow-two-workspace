...
- must create a domain enum in the [`domain` layer](../../mla/architecture/architecture.md), at
...
- must key a display or a payload off the enum, never off a parallel list
  ([enum projections](../../mla/constructs/data/enum-projections.md)) — the suffixes and their layer homes are
...
- [constructs](../../mla/constructs/constructs.md) — the rule against fanning an enum into parallel `is*` props
