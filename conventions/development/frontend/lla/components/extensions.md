...
- **No `class` in this role** — a `const` object has no constructor and cannot be instantiated, which is
  the whole point of an extension. The construct itself is allowed elsewhere
  ([constructs](../constructs/constructs.md)), for an `Error` subclass or a chained builder.
