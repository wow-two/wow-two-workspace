...
> Use case — adding a host, registering a dependency, or changing what the pipeline does before a handler runs.

## What lives here

- [host configuration](host-configuration.md) — the `Configure` / Extensions split, settings binding, env overrides
...
- must keep a rule that is true before any feature exists here; a feature-shaped rule belongs to its domain.
