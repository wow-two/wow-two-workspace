...
`AppErrorProblemDetailsFactory.Create(error, httpContext, statusMapper, messageMapper)` is the single builder, used by **both** the controller failure-arm (`.Match`) and the global exception handlers. It:

- sets `status` (via `IErrorHttpStatusCodeMapper`), `detail` (via `IErrorMessageMapper`), `code` (= `error.Type`), `type` (`urn:wow-two:error:{Type}`).
...
- [result-pattern.md](../../constructs/data/result.md) · [validation.md](../../constructs/behavior/validator.md) · [controllers.md](../../constructs/behavior/controller.md)
