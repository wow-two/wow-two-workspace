...
> once at the host, never per controller. The frontend mirrors it verbatim: [type mapping](../../../../../frontend/mla/domains/api/type-mapping.md).
...
> ([enums](../../components/enums.md)). One enum → `active` on the wire, `active` in the column, `Active` in code.

---

## Wiring

- must apply the contract once, at the host — `AddControllers().AddJsonStringEnums()` over the SDK preset (`JsonOptionsPresets.Default`), never a hand-rolled `JsonSerializerOptions` per controller ([host configuration](../startup/host-configuration.md)).
