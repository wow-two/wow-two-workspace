...
> Dapper query/command classes for direct SQL, plus the SDK's generic `DapperRepository<TEntity, TId>` for single-table CRUD; no `IQueryable` /
> Specification layer.
> Purpose — keep reads explicit and SQL-visible: generated CRUD where it's trivial, hand-written SQL everywhere else, with no hidden query translation.
> Use case — reach for it in any `{Repo}.Persistence` assembly when a service needs to read or write rows against a relational store.
...
- `/// <summary>` starts with **Accesses** — the data-access starter, whichever direction the class runs in.
...
/// <summary>Accesses status changes for supply source URLs.</summary>
