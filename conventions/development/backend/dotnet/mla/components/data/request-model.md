...
> **API request** — the presentation-layer body a client sends, named `{Verb}{Noun}ApiRequest`; a controller binds it and maps it to its **application request** (the mediator `Command` / `Query` — see [mediator](../../domains/messaging/mediator.md)).
...
- must be named `{Verb}{Noun}ApiRequest`, **verb-first** — `CreateCodeApiRequest`, `UpdateCodeApiRequest`. An api request exists for one controller action, so it reads like the action it binds to; the **application request** is the noun-first one, because those are searched by domain ([mediator](../../domains/messaging/mediator.md) § *The application request*)
...
- `required` on every non-nullable property; each carries a `Gets {what}.` summary (property doc rule → [summary](../../../lla/notation/documentation/summary.md))
...
- may use `=>` for a member that returns or delegates — a request body carries data, and its members do not grow
  ([style](../../../lla/notation/style/style.md) § *The body*); its `ToCommand` stays a block body
...
- class summary starts with **Extends** - `Extends <see cref="{Request}"/> for application-request mapping.` ([summary](../../../lla/notation/documentation/summary.md) § *Extension classes*)
...
- the mapping method **builds + returns** the application request — block body, not `=>` ([members.md](../../../lla/constructs/constructs.md))
